...
public class WebConfig implements WebMvcConfigurer{
	 ...
	 * 
	 * 	Spring 프레임워크를 거치지 않고 바로 응답되게 할수있다.
	 * 
	 * 	- SmartEditor2 나 프로필 이미지를 저장할 upload 폴더를 만들 예정
	 */
	
	//여기서 bean 으로 관리되는 MyInterceptor 객체가 필요하다면 ...
	@Autowired MyInterceptor inter;
	
	//@Autowierd 어노테이션에서 주입 받은 객체를 이 메소드에서 활용하면 된다.
	@Override
	public void addInterceptors(InterceptorRegistry registry) {
		registry.addInterceptor(inter).addPathPatterns("/users/*");
	}
