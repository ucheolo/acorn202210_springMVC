...
public class TestController {
	...
	public String test() {
		/*
		 * 	Controller 메소드에서 return type 을 String 으로 설정한 후
		 * 	문자열을 리턴해주면서 해당 문자열은 view page (jsp) 페이지의 위치가 된다.
	...
	public ModelAndView fortune() {
		//Model 과 view page 의 위치를 동시에 담을 수 있는 ModelAndView 객체 생성
		ModelAndView mView = new ModelAndView();
		
		//view page 에 전달할 모델(데이터) 라고 가정하자
		String fortuneToday = "동쪽으로 가면 귀인을 만나요!";
		
		// HttpServletRequest 객체에 담는 대신 .addObject(key, value) 형태로 ModelAndView에 담으면 된다.
	...
	public ModelAndView fortune2(ModelAndView mView) {
		...
		//view page 에 전달할 모델(데이터) 라고 가정하자
		String fortuneToday = "동쪽으로 가면 귀인을 만나요!222";
	...
	public ModelAndView fortune3() {
		...
		//view page 에 전달할 모델(데이터) 라고 가정하자
		String fortuneToday = "동쪽으로 가면 귀인을 만나요!333";
